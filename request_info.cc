...
void extractServiceName(StringView host,
                        StringView destination_namespace,
...
void getDestinationService(StringView dest_namespace,
...
int64_t RequestInfoImpl::requestTimestamp() {
  if (!request_info_.has_request_timestamp()) {
    int64_t request_timestamp;
    getValue({"request", "time"}, &request_timestamp);
    request_info_.mutable_request_timestamp()->set_value(request_timestamp);
  }
  return request_info_.request_timestamp().value();
}

int64_t RequestInfoImpl::responseTimestamp() {
  if (!request_info_.has_response_timestamp()) {
    int64_t response_timestamp;
    getValue({"response", "time"}, &response_timestamp);
    request_info_.mutable_response_timestamp()->set_value(response_timestamp);
  }
  return request_info_.response_timestamp().value();
}
...
int64_t RequestInfoImpl::duration() {
  if (!request_info_.has_duration()) {
    ...
    request_info_.mutable_duration()->set_value(duration);
  }
  return request_info_.duration().value();
}
...
int64_t RequestInfoImpl::responseDuration() {
  ...
  return request_info_.response_duration().value();
}
